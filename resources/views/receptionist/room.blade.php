...
<div class="p-6 max-w-7xl mx-auto">
  ...
  <div class="flex justify-between items-end mb-8">
    ...
    <a href="{{ route('createRoom') }}"
      ...
      <span class="w-5 h-5 bg-white/25 rounded-full flex items-center justify-center text-base leading-none">+</span>
      Add Room
    </a>
  </div>
  ...
  <form method="GET"
    ...
    <button class="bg-gray-900 hover:bg-black text-white rounded-lg px-4 py-2.5 text-sm
        ...
        <tr class="hover:bg-gray-50/60 transition">
          ...
          <td class="px-6 py-4">
            ...
            @elseif($room->type === 'Double')
              <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-pink-50 text-pink-800">
                Double
              </span>
            @else
          ...
          <td class="px-6 py-4">
            ...
            @elseif($room->status === 'occupied')
              <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-red-50 text-red-800">
                <span class="w-1.5 h-1.5 rounded-full bg-red-500"></span> Occupied
              </span>
            @else
              <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-amber-50 text-amber-800">
          ...
          <td class="px-6 py-4">
            <div class="flex justify-end gap-2">
              ...
              <form action="{{ route('destroyRoom', $room->id) }}" method="POST"
                ...
                @csrf
                @method('DELETE')
                <button class="px-3 py-1.5 text-xs font-medium rounded-lg
